...
public class AgregarCentro extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //Pantalla agregar centro
        setContentView(R.layout.activity_agregar_centro);

        // Los elementos que se mostrarán en la pantalla para agregar un centro
        ...
        // Abrir GALERIA
        btnFoto.setOnClickListener(v -> { Intent intent = new Intent(Intent.ACTION_PICK);
            intent.setType("image/*");
            startActivityForResult(intent, PICK_IMAGE);
        });

        // Guardar foto
        btnGuardar.setOnClickListener(v -> { String nombreStr = nombre.getText().toString().trim();String direccionStr = direccion.getText().toString().trim(); String telefonoStr = telefono.getText().toString().trim();
            ...
            // Si el administrador subio una foto del centro, se sube primero. Sino, se sube el centro sin foto
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode == PICK_IMAGE && resultCode == Activity.RESULT_OK && data != null) { fotoSeleccionada = data.getData(); ImageView foto = findViewById(R.id.ivFotoCentro);foto.setImageURI(fotoSeleccionada);
        }
    }

    // Se sube la foto a Cloudinary y luego se guarda el centro
    ...
    // Se guardan los datos del centro en firestore
