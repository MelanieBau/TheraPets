...
public class GestionAnimales extends AppCompatActivity {
    ...
    private void mostrarDialogoEditar(Animal animal) {
        ...
        new AlertDialog.Builder(this).setTitle("Editar animal").setView(dialogView).setPositiveButton("Guardar", (dialog, which) -> {
            ...
            datos.put("centro", centroId);

            db.collection("animales").document(animal.getId()).update(datos).addOnSuccessListener(a -> cargarAnimales());
    ...
    private void borrarAnimal(Animal animal) {
        new AlertDialog.Builder(this).setTitle("Borrar animal").setMessage("¿Borrar a " + animal.getNombre() + "?").setPositiveButton("Borrar", (dialog, which) ->
                db.collection("animales").document(animal.getId()).delete().addOnSuccessListener(a -> cargarAnimales())
        ).setNegativeButton("Cancelar", null).show();
    }
